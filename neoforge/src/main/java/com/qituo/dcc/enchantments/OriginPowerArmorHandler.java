...
public class OriginPowerArmorHandler {
    ...
    /**
     * 计算玩家护甲上的始源之力总等级
     * 直接读 NBT，避免依赖 JSON 数据驱动附魔还未就位的 Holder 注册
     */
    public static int getTotalArmorLevel(Player player) {
        int total = 0;
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            if (slot.isArmor()) {
                total += ModEnchantments.getOriginPowerLevel(player.getItemBySlot(slot));
            }
        }
        return total;
    }
    ...
    private static void applyDamageReflection(Player player, LivingDamageEvent.Pre event, int totalLevel) {
